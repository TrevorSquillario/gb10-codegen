...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        -c|--copy-to|--copy-to-host|--copy-to-hosts)
            COPY_TO_FLAG=true
            # Expect a single argument (comma-separated list of hosts) after the flag.
            shift
            if [[ "$#" -gt 0 && "$1" != -* ]]; then
                add_copy_hosts "$1"
            else
                echo "Error: --copy-to requires an argument."
                usage
            fi
            ;;
...
# If --copy-to was used, COPY_HOSTS should have been populated by add_copy_hosts
if [ "$COPY_TO_FLAG" = true ] && [ "${#COPY_HOSTS[@]}" -eq 0 ]; then
    echo "Error: No hosts specified with --copy-to."
    exit 1
fi
